;(function(){
	...
	globalTimer:"",
		...
		DrawCuerpo : function(){
				...
				cuerpoHTML += '</div>';
				cuerpoHTML += (setting.tickertournament!==0) ? '<div class="wdg_matchesresult_01_bottom" style="height:0px;">' : '<div class="wdg_matchesresult_01_bottom">';
				cuerpoHTML += (setting.tickertournament!==0) ? '<div class="wdg_matchesresult_important" id="FListTournaments" style="display:none!important">': '<div class="wdg_matchesresult_important" id="FListTournaments">';
				cuerpoHTML += '</div>';
			...
			.done(function(data) {
				...
				$("#FListTournaments a, #ListTournaments a").bind('click', function(event) {
					...
					if(!$(this).hasClass('onShowItem')){
						...
						$("#FListTournaments").parent().find('a').css('color', '#8f8f8f').removeClass('onShowItem');
		...
		DrawContentFirst: function(contenido,tipo){			
			...
			for (var y = 0; y < contenido.length; y++) {
				if(y<16){
					...
					ItemView += '<div class="wdg_match_01_time background-color1a">';
					ItemView += '<p>';
					ItemView += '<a class="textcolor-title5a">'+contenido[y].periodabrev+' '+(isNaN(contenido[y].time) ? contenido[y].time+'"' : '')+'</a>';
					ItemView += '</p>';
					ItemView += '</div> ';
					...
					ItemView += '</div>';
					ItemView += '</div>';
					ItemView += '<div class="shadow"></div><div class="wdg_match_01_link">';
					...
					ItemView += '<a class="textcolor-title1a" href="'+contenido[y].Website+'">'+contenido[y].txtLink.substring(0,numSplit)+'</a>';			
					ItemView += '</p>';
					ItemView += '</div>';
		...
		updateGoles:function(data){
			...
			for (var o = 0; o < data.matches.match.length; o++) {
				...
				tituloAct = selectorTMP.find(".textcolor-title5a").text();
				...
				if(tituloNue!==tituloAct){
					selectorTMP.find(".textcolor-title5a").css({'display':'none','position':'relative'}).text(tituloNue).fadeIn('slow');
				}
		...
		resize: function(){
			...
			if ($(window).width()>=933) {
				$(".wdg_matchesresult_todos").show();
					var aplicando = $('.wdg_matchesresult_01 .wdg_matchesresult_01_container .wdg_matchesresult_01_right .wdg_matchesresult_01_components, .wdg_matchesresult_01 .wdg_matchesresult_01_container .wdg_matchesresult_01_right .wdg_matchesresult_01_components ul');
					var numeroItems = $("#listNow li").size();
					...
					aplicando.css('height',heightModulo+'px');
...
;jQuery(function($){ 
    (function(T, $) {
        ...
        $hide.click(function(e) {
            ...
            var visShow = $show.css('visibility');
			 if (T.getDeviceSize() === 'large') {
			 $('.wdg_matchesresult_01 .wdg_matchesresult_01_left .wdg_matchesresult_visible').css('margin-top','9%');
			 }
        ...
        $show.click(function(e) {
            ...
            if (T.getDeviceSize() === 'large') {
				...
				$('.wdg_matchesresult_01 .wdg_matchesresult_01_container .wdg_matchesresult_01_right .wdg_matchesresult_01_components').css('height',heightModuloc+'px');
                $closeElement.animate({
                    'height': heightModulo
                }, animationDelay);
            }
            if (T.getDeviceSize() === 'medium') { 
            $('.wdg_matchesresult_01 .wdg_matchesresult_01_left .wdg_matchesresult_visible').css('margin-top',porce+'%');           	
                $closeElement.animate({
                    'height': 460
                }, animationDelay);              
            }
